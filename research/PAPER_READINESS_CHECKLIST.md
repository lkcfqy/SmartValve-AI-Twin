...
- v0.4 bearing-protocol paper status: scientific family, final artifacts, empirical manuscript,
  amendment-002 local quality gate, and working-PDF preflight complete; release 0.11.1 and 0.11.2
  archive-source failures are retained, while amendment 003/EXP-525 focus-test the 0.11.3
  correction. Its final full gate, archive validation, clean reproduction, public revision/DOI,
  and external human review remain
...
| Final local technical quality package | eight fixed checks, JUnit/coverage evidence, independent log/hash validator, release inclusion | **amendment-002 pass; amendment-003 rerun pending**, EXP-518/519; 427/427 tests, zero failures/errors/skips, `78.161188%` coverage, 8/8 checks and independent hash/log validation; EXP-525 then passed the focused 0.11.3 correction, but a new full pair is still required |
| Dependency and static security preflight | pinned build, runtime, CPU-CI, and GPU-research locks plus application/project-script Bandit | **amendment-002 local pass; amendment-003 rerun pending**; EXP-518/519 passed expanded-scope medium/high Bandit and strict runtime/build, CPU-CI, and GPU-research audits; this is not a detached clean reproduction |
...
| Full quality and clean-environment reproduction | Ruff/tests/coverage plus independent environment | **local amendment-002 gate pass; clean reproduction not yet passed**; EXP-518/519 passed 427/427 with zero skips and `78.161188%` coverage. EXP-524's fresh 0.11.2 run also passed 427/427, `78.075698%`, security and dependency checks, then correctly failed at the raw-free artifact boundary; the 0.11.3 full chain and formal detached-commit run remain open |
| Container/Compose CI gate | Compose interpolation and reproducible application-image build | **Compose static pass; image build open**; Windows Compose v5.3.0 validated base, TLS, and full SSO overlays without starting services, but the Docker Desktop Linux engine remained unavailable; CI now requires all three configuration checks plus the image build |
| Clean-reproduction logistics | detached commit, hashed release, locked environment, QA, artifact and archive validation | **two failures retained; 0.11.3 focused correction passed**; EXP-514 failed 7/427 from 0.11.1, EXP-524 passed the complete quality prefix from 0.11.2 but failed its raw-free regeneration contract, and amendment 003/EXP-525 bound the correction without weakening default checks; complete 0.11.3 and author-approved detached commit/DOI runs remain open |
...
- A final independently reopened 0.11.3 archive and a complete passing archive-source clean run;
  0.11.1 and 0.11.2 are retained but withdrawn after EXP-514 and EXP-524 respectively.
- A detached clean-environment reproduction from an author-approved frozen commit.
- Advisor/domain-expert and independent statistical review.
- An author-approved public commit and archival code/artifact identifier.
...
sensitivity, and unusually strong audit trail. Publication level still depends on detached
reproduction, public freezing, human review, and venue judgment; no tier or acceptance is
guaranteed.
...
- [ ] Run the post-amendment-003 full quality pair, then build, independently reopen, and completely
      clean-run release 0.11.3.
- [ ] Run a detached clean-environment reproduction from the author-approved frozen revision.
- [ ] Have an advisor/domain expert and independent statistician review the estimand and claims.
- [ ] Create an author-approved public commit and archival artifact release.
...
> and working-PDF evidence supports a credible top-domain-journal protocol paper, but the corrected
> archive must still pass full clean reproduction, and an author-approved public revision/DOI,
> venue-final files, and external human review remain; this is not a top-tier algorithm submission.
